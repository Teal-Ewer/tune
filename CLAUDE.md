...
- **Persistence**: `units`, `targetHz`, and `buffers` are persisted to `localStorage` under the key `brass-tuner-v1`. Indoor/outdoor temp and humidity are intentionally *not* persisted — they're conditions for one performance, not user preferences.
- **Core physics** (`speedOfSound`, `sectionResult`): speed of sound is computed in Celsius; the F/C unit toggle is presentation-only and converts existing input values when flipped. The per-section result uses a "buffer factor" β to weight how much of the air column tracks ambient temperature vs. breath temperature: `adjRatio = 1 + β * (cOutdoor/cIndoor - 1)`, then `tuneTo = targetHz / adjRatio`.
...
- **Temperature input ranges** differ for indoor (`tempMaxIndoor`) vs. outdoor (`tempMaxOutdoor`) — outdoor goes higher because performance fields routinely exceed reasonable indoor temperatures.
